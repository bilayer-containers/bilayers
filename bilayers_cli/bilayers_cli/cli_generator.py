...
from bilayers_cli.parse import Config, HiddenArgs
...
def generate_cli_command(parsed_config: Config) -> list[str]:
    ...
    for key, param in cli_tags.items():
        ...
        folder_name: str | None = param.get("folder_name")
        ...
        if param_type == "checkbox":
            append_value: bool | None = param.get("append_value", False)
            if append_value:
    ...
    hidden_args: dict[str, HiddenArgs] | None = exec_function.get("hidden_args", {})
    ...
    for arg in hidden_args.values(): # pyright: ignore
        ...
        hidden_append_value : bool | None = arg.get("append_value")
...
def main(parsed_config: Config, return_as_string: bool) -> str | list[str]:
    """
